...
#train multi-task autoencoder for spatial filtering
    ...
    if retrain_sf_flag:
        ...
        # data_train_sf = generate_training_data_sf_AI(M, N, d, wavelength, SNR_sf, doa_min, NUM_REPEAT_SF, grid_sf,
        #                                              GRID_NUM_SF,
        #                                              output_size_sf, SF_NUM, SF_SCOPE, MC_mtx, AP_mtx, pos_para)
        data_train_sf = read_all_data_and_labeling(CONFIG_DATA_FOLDER)
        tf.reset_default_graph()
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(num_epoch_sf):
                [data_batches, label_batches] = generate_spec_batches(data_train_sf, batch_size_sf)
    ...
    if retrain_ss_flag == True:
        ...
        # data_train_ss = generate_training_data_ss_AI(M, N, K_ss, d, wavelength, SNR_ss, doa_min, doa_max, step_ss,
        #                                              doa_delta,
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(num_epoch_ss):
                ...
                for batch_idx in range(len(data_batches)):
                    ...
                    _, loss_ss, accuracy_ss = sess.run([enmod_2.train_op_ss, enmod_2.loss_ss, enmod_2.accuracy_ss],
                                                       feed_dict=feed_dict)

                    print('Epoch: {}, Batch: {}, loss: {:g}, accuracy: {}'.format(epoch, batch_idx, loss_ss, accuracy_ss))
    ...
    #         # test_cov_vector = generate_array_cov_vector_AI(M, N, d, wavelength, test_DOA, test_SNR, MC_mtx, AP_mtx,
    #         #                                                pos_para)
